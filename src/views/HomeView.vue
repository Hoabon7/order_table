<template>
  <div class="hello">
    <div v-for="item in customers" :key="item.id" class="cl_item">
      <h3>{{ item.name }}</h3>
      <h4>{{ item.email }}</h4>
      <button class="delete_button" @click.prevent="deleteCustomer(item.id)">
        DELETE ITEM
      </button>
  ...
  <div>
    <router-link active-class="active" exact :to="{ name: 'add_customer' }">
      Add Customer
    </router-link>
  </div>
</template>
...
import { DELETE_CUSTOMER } from "../store/action.type";
